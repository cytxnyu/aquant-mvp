...
TOOL_REGISTRY: tuple[ToolCapability, ...] = (
    ...
    ToolCapability(
        "text_intelligence_registry",
        "python run_mvp.py discover-text-intelligence",
        "foundation",
        "implemented",
        "aquant_mvp.events.text_intelligence",
        "text_intelligence_registry.csv, text_intelligence_registry.md",
        False,
        "low",
        "Optional news/text intelligence bases for embeddings, event classifiers, summarizers, and vector stores.",
    ),
    ToolCapability(
        "similar_event_evidence",
        "python run_mvp.py build-event-store --source sample --allow-sample --universe 000630,601899,600362",
        "explainability",
        "implemented",
        "aquant_mvp.events.text_intelligence",
        "similar_events.csv/json/md, stock_similar_events.csv/json/md",
        False,
        "low",
        "PIT-safe similar-news/event retrieval for explanation evidence; never a direct trading signal.",
    ),
    ToolCapability(
        "event_impact_study",
        "python run_mvp.py analyze-event-impact --universe 000630,601899,600362",
        "explainability",
        "implemented",
        "aquant_mvp.events.impact",
        "event_impact_study_returns.csv, event_impact_study_cohorts.csv, event_impact_study_pit_priors.csv, event_impact_study.md",
        False,
        "low",
        "Historical event-outcome cohort and PIT-prior diagnostics for validating news/event factors; never a direct trading signal.",
    ),
    ToolCapability(
        "tool_registry",
